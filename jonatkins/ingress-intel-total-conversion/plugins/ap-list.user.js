...
window.plugin.apList.handleDestroyPortal = function () {
    ...
    plugin.apList.destroyPortalsGuid = $.grep(plugin.apList.destroyPortalsGuid, function (portalGuid, ind) {
        ...
        if (!portal || plugin.apList.portalSide(portal) !== enemy)
            return false;
        return true;
    });
    ...
    $.each(plugin.apList.sortedPortals[enemy], function (ind, portal) {
        ...
        if (portal.portalV2.linkedEdges)
            newPortal.portalV2.linkedEdges = newLinkedEdges;
        if (portal.portalV2.linkedFields)
            newPortal.portalV2.linkedFields = newLinkedFields;
