...
function Feed(link, drive, opts) {
    if (!(this instanceof Feed))
        return new Feed(link, drive, opts);
    if (!opts)
        opts = {};
    var self = this;
    this.id = link;
...
Feed.prototype.get = function (index, cb) {
    ...
    this.open(function (self) {
        if (self._channel.blocks && index >= self._channel.blocks)
            return cb(null, null);
        if (self._channel.bitfield.get(index))
            return self._block(index, cb);
...
function decoder(cb) {
    return function (err, value) {
        ...
        // TODO: move to module
        if (entry.type === 'file')
            entry.value = messages.File.decode(entry.value);
        cb(null, entry);
    };
}
function Cursor(feed) {
    ...
    this._indexSize = 0;
    this._end = 0;
    if (this._feed._channel.blocks || this._feed._blocks)
        this._onblocks();
}
...
Cursor.prototype.read = function (offset, cb) {
    if (this.position.bytes === offset)
        this.next(cb);
    else
        this.seekAndRead(offset, cb);
};
...
Cursor.prototype.next = function (cb) {
    ...
    if (!inited)
        return this.seekAndRead(offset, cb);
    var self = this;
...
Cursor.prototype.seek = function (offset, cb) {
    ...
    this._block(this._end + i, function (err, dir) {
        if (err)
            return cb(err);
        if (!dir)
            return cb(new Error('Missing block directory'));
        var len = 0;
